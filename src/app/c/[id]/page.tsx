...
export async function generateMetadata(
  ...
  // Start with a harmless default; we'll improve it if Supabase works
  let title = `Shared comparison ${id.slice(0, 6)}… — CompareAnything`;
  let description =
    ...
    const { data, error } = await supabaseServerClient
      ...
      .eq("id", id)
      .maybeSingle<Pick<ComparisonRow, "item_a" | "item_b" | "result">>();

    if (!error && data) {
      const itemA = data.item_a ?? "Thing A";
      const itemB = data.item_b ?? "Thing B";

      title = `${itemA} vs ${itemB} — CompareAnything`;

      try {
        const parsed =
          typeof data.result === "string"
            ? JSON.parse(data.result as any)
            : data.result;

        if (parsed && typeof (parsed as any).summary === "string") {
          description = (parsed as any).summary;
        } else {
          description = `Compare ${itemA} and ${itemB} side by side on what really matters.`;
        }
      } catch {
        description = `Compare ${itemA} and ${itemB} side by side on what really matters.`;
      }
    ...
    console.error("Error in generateMetadata for /c/[id]:", err);
  }
